  ...
  return (
    <div className={`overflow-hidden select-none h-screen w-screen flex flex-col transition-colors duration-300 ${isLight ? 'bg-slate-50 text-slate-800' : 'bg-slate-950 text-slate-100'}`}>
      ...
      <div className="relative flex-1 w-full h-full overflow-hidden flex">
        ...
        <div className={`absolute lg:relative z-20 h-full overflow-visible transition-all duration-300 shrink-0 shadow-2xl ${isSidebarOpen ? 'translate-x-0 lg:w-[430px]' : '-translate-x-full lg:w-0 lg:translate-x-0'}`}>
          <div className={`h-full w-[min(430px,calc(100vw-2rem))] transition-transform duration-300 ${isSidebarOpen ? 'translate-x-0' : 'lg:-translate-x-full'}`}>
            <Sidebar />
          </div>
        </div>
